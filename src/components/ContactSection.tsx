          ...
          <>
            ...
            <form onSubmit={handleSubmit} className="space-y-10 sm:space-y-12 animate-in fade-in duration-700">
              <div className="space-y-8 sm:space-y-10">
                <div className="space-y-2"><label htmlFor="contact-full-name" className="text-xs font-bold uppercase tracking-widest text-[#302D28]">{clientType === 'particular' ? 'Nombre completo' : 'Empresa / Razón Social'}</label><input id="contact-full-name" name="fullName" type="text" autoComplete="name" value={formData.fullName} onChange={e => { setFormData(prev => ({ ...prev, fullName: e.target.value })); if (errors.fullName) setErrors(prev => ({ ...prev, fullName: undefined })); }} aria-invalid={Boolean(errors.fullName)} aria-describedby={errors.fullName ? 'contact-full-name-error' : undefined} className={`w-full bg-transparent border-b px-0 py-3 text-base sm:text-lg text-[#302D28] placeholder-[#302D28]/30 focus:outline-none focus:border-[#7F203D] focus-visible:ring-2 focus-visible:ring-[#7F203D] focus-visible:ring-offset-2 transition-colors font-light ${errors.fullName ? 'border-red-500' : 'border-[#302D28]/25'}`} placeholder={clientType === 'particular' ? 'Ej. Juan Pérez' : 'Ej. Corporación ABC'} />{errors.fullName && <p id="contact-full-name-error" className="text-xs text-red-600 pt-1" role="alert">{errors.fullName}</p>}</div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-12"><div className="space-y-2"><label htmlFor="contact-phone" className="text-xs font-bold uppercase tracking-widest text-[#302D28]">WhatsApp / Teléfono</label><input id="contact-phone" name="phone" type="tel" autoComplete="tel" value={formData.phone} onChange={e => { setFormData(prev => ({ ...prev, phone: e.target.value })); if (errors.phone) setErrors(prev => ({ ...prev, phone: undefined })); }} aria-invalid={Boolean(errors.phone)} aria-describedby={errors.phone ? 'contact-phone-error' : undefined} className={`w-full bg-transparent border-b px-0 py-3 text-base sm:text-lg text-[#302D28] placeholder-[#302D28]/30 focus:outline-none focus:border-[#7F203D] focus-visible:ring-2 focus-visible:ring-[#7F203D] focus-visible:ring-offset-2 transition-colors font-light ${errors.phone ? 'border-red-500' : 'border-[#302D28]/25'}`} placeholder="+54 9 261 512-3456" />{errors.phone && <p id="contact-phone-error" className="text-xs text-red-600 pt-1" role="alert">{errors.phone}</p>}</div>{clientType === 'empresa' && <div className="space-y-2"><label htmlFor="contact-email" className="text-xs font-bold uppercase tracking-widest text-[#302D28]">Correo Electrónico Corporativo</label><input id="contact-email" name="email" type="email" autoComplete="email" value={formData.email} onChange={e => { setFormData(prev => ({ ...prev, email: e.target.value })); if (errors.email) setErrors(prev => ({ ...prev, email: undefined })); }} aria-invalid={Boolean(errors.email)} aria-describedby={errors.email ? 'contact-email-error' : undefined} className={`w-full bg-transparent border-b px-0 py-3 text-base sm:text-lg text-[#302D28] placeholder-[#302D28]/30 focus:outline-none focus:border-[#7F203D] focus-visible:ring-2 focus-visible:ring-[#7F203D] focus-visible:ring-offset-2 transition-colors font-light ${errors.email ? 'border-red-500' : 'border-[#302D28]/25'}`} placeholder="nombre@empresa.com" />{errors.email && <p id="contact-email-error" className="text-xs text-red-600 pt-1" role="alert">{errors.email}</p>}</div>}</div>
                <div className="space-y-3"><span className="text-xs font-bold uppercase tracking-widest text-[#302D28] block">Modalidad de consulta</span><div className="grid grid-cols-1 sm:grid-cols-3 gap-2">{([['presencial','Presencial'],['videollamada','Videollamada'],['telefonica','Telefónica']] as const).map(([value,label]) => <button key={value} type="button" aria-pressed={consultationMode === value} onClick={() => setConsultationMode(value)} className={`py-3 px-3 border text-xs font-bold uppercase tracking-wider transition-colors ${consultationMode === value ? 'border-[#7F203D] bg-[#7F203D]/10 text-[#7F203D]' : 'border-[#302D28]/20 text-[#302D28]/70 hover:border-[#7F203D]/50'}`}>{label}</button>)}</div></div>
                <div className="space-y-2"><label htmlFor="contact-message" className="text-xs font-bold uppercase tracking-widest text-[#302D28]">Motivo de consulta</label><textarea id="contact-message" name="message" rows={4} value={formData.message} onChange={e => { setFormData(prev => ({ ...prev, message: e.target.value })); if (errors.message) setErrors(prev => ({ ...prev, message: undefined })); }} aria-invalid={Boolean(errors.message)} aria-describedby={errors.message ? 'contact-message-error' : undefined} className={`w-full bg-transparent border-b px-0 py-3 text-base sm:text-lg text-[#302D28] placeholder-[#302D28]/30 focus:outline-none focus:border-[#7F203D] focus-visible:ring-2 focus-visible:ring-[#7F203D] focus-visible:ring-offset-2 transition-colors font-light resize-none leading-relaxed ${errors.message ? 'border-red-500' : 'border-[#302D28]/25'}`} placeholder="Describa brevemente los hechos o la situación procesal..." />{errors.message && <p id="contact-message-error" className="text-xs text-red-600 pt-1" role="alert">{errors.message}</p>}</div>
