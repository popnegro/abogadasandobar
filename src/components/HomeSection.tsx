  ...
  return <div id="home-section" className="relative w-full">
    <section id="hero-banner" aria-labelledby="hero-title" className="relative flex min-h-[600px] w-full items-center justify-center overflow-hidden pb-16 pt-28 lg:min-h-[640px] lg:pb-20 lg:pt-32">
      <div className="absolute inset-0" aria-hidden="true">
        <img src={ASSETS.heroOffice} alt="" width={1376} height={768} loading="eager" fetchPriority="high" decoding="async" className="h-full w-full object-cover object-center" />
      </div>
      <div className="absolute inset-0 bg-gradient-to-br from-[#181614]/95 via-[#231F1C]/90 to-[#2A2522]/80" aria-hidden="true" /><div className="relative z-10 mx-auto flex w-full max-w-7xl flex-col justify-center px-4 sm:px-6 lg:px-8"><div className="grid grid-cols-1 items-center gap-8 lg:grid-cols-12 lg:gap-12"><div className="space-y-6 lg:col-span-8 lg:space-y-8"><div className="animate-in fade-in slide-in-from-bottom-4 duration-700"><h1 id="hero-title" className="mb-3 font-serif text-2xl font-bold leading-tight text-white sm:text-2xl lg:text-3xl">Abogada Emilia Sandobar</h1><h2 className="mb-6 max-w-4xl font-serif text-4xl font-bold leading-[1.05] tracking-tight text-white sm:text-5xl lg:text-6xl">Defensa penal y representación de víctimas <span className="text-[#D9A9B8]">en Mendoza</span></h2><p className="max-w-2xl text-base font-light leading-relaxed text-white sm:text-lg lg:text-xl">Emilia Sandobar es abogada penalista en Mendoza especializada en derecho penal. Ofrece defensa penal, representación de víctimas y atención 24 hs para urgencias, con estrategia jurídica rigurosa e intervención profesional directa.</p></div></div><div className="mx-auto w-full max-w-md lg:col-span-4 lg:ml-auto lg:mr-0"><div className="space-y-6 border border-white/20 bg-[#181614]/80 p-6 backdrop-blur-md sm:p-8"><h2 className="font-serif text-xl font-bold leading-tight text-white sm:text-2xl">¿Necesita asistencia jurídica?</h2><div className="space-y-3"><button type="button" onClick={handleNavigateToContact} className={`flex w-full cursor-pointer items-center justify-center gap-2 bg-[#7F203D] px-8 py-3.5 text-sm font-bold uppercase tracking-widest text-white shadow-md transition-colors hover:bg-[#691931] ${lightFocusRing}`}><span>Guardia Legal 24 hs.</span><ArrowRight className="h-4 w-4 shrink-0" aria-hidden="true" /></button><button type="button" onClick={() => handleNavigateToServices()} className={`flex w-full cursor-pointer items-center justify-center gap-2 border border-white/50 bg-transparent py-3 text-xs font-bold uppercase tracking-widest text-white transition-colors hover:bg-white/10 ${lightFocusRing}`}><span>Ver servicios</span></button></div></div></div></div></div></section>
