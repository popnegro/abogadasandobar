...
const METHOD_STEPS = [
  ...
  },
  {
    step: '04',
    title: 'Actuamos',
    desc: 'Ejecutamos la hoja de ruta con firmeza técnica en sede judicial o en mesas de negociación.',
  },
] as const;
  ...
  <div id="method-section" className="w-full bg-[#FAF6F0]">
    ...
    >
      <div
        ...
        style={{ backgroundImage: `url(${ASSETS.heroMethod})` }}
        aria-hidden="true"
      />
      ...
      <div className="relative z-10 mx-auto w-full max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="mb-8 lg:mb-10">
          <Breadcrumb
            items={[{ label: 'Nuestro método', active: true }]}
            onNavigate={setActiveTab}
            variant="primary"
          />
        </div>
        <div className="max-w-4xl space-y-6">
          <h1 id="method-hero-title" className="font-serif text-4xl font-bold leading-[1.05] tracking-tight text-white sm:text-5xl lg:text-6xl">
            Nuestro método
            <span className="block text-[#D9A9B8]">jurídico</span>
          </h1>
    ...
    <main className="mx-auto max-w-7xl px-4 py-16 sm:px-6 lg:px-8 lg:py-24">
      <section id="metodo-proceso" aria-labelledby="method-title" className="space-y-12 lg:space-y-16">
        <div className="flex items-baseline justify-between border-t border-[#302D28]/20 pt-8">
          <h2 id="method-title" className="text-[10px] font-bold uppercase tracking-widest text-[#7F203D] sm:text-xs">
            Método y proceso
          </h2>
        </div>
            ...
            <article key={item.step} className="group w-[85vw] shrink-0 snap-center sm:w-auto sm:shrink sm:snap-align-none">
              <div className="relative h-full w-full border-t border-[#302D28]/20 pr-8 pt-6 sm:pt-8 lg:pr-12">
                ...
                <div className="space-y-4">
                  <span className="block font-serif text-sm font-bold uppercase tracking-widest text-[#7F203D] lg:text-base">Etapa {item.step}</span>
                  <h3 className="font-serif text-xl font-bold text-[#302D28] transition-colors group-hover:text-[#7F203D] sm:text-2xl">{item.title}</h3>
                  <p className="max-w-[280px] text-sm font-light leading-relaxed text-[#302D28]/70 sm:max-w-none sm:text-base">{item.desc}</p>
                </div>
              </div>
            </article>
          ))}
      ...
      <section aria-labelledby="method-consultation-title" className="pt-16 lg:pt-24">
        <div className="flex flex-col gap-6 border border-[#DDD2C5] bg-[#FFF8F2] p-6 sm:p-8 lg:flex-row lg:items-center lg:justify-between lg:gap-10">
          <div className="max-w-3xl space-y-2">
            ...
            <h2 id="method-consultation-title" className="font-serif text-2xl font-bold leading-tight text-[#302D28] sm:text-3xl">¿Necesita evaluar una situación jurídica específica?</h2>
            <p className="text-sm font-light leading-relaxed text-[#302D28] sm:text-base">Cada procedimiento tiene singularidades únicas. Exponga su caso con absoluta reserva.</p>
          </div>
          <button type="button" onClick={onOpenConsultationModal} className="group inline-flex w-full cursor-pointer items-center justify-center gap-3 border border-[#7F203D] bg-[#7F203D] px-6 py-3.5 text-xs font-bold uppercase tracking-widest text-white transition-colors hover:border-[#691931] hover:bg-[#691931] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#7F203D] focus-visible:ring-offset-2 sm:w-auto sm:text-sm">
